...
// import { Search, Sparkles, ChevronDown, MapPin, Loader2, Building2, IndianRupee, Maximize2, SlidersHorizontal, Check, Users, DollarSign, Euro } from "lucide-react";
...
//   /* =========================================================FETCH CITIES FROM CRM + AUTO SELECT USER CITY========================================================= */
//   useEffect(() => {
...
//   /* ==============================================================AUTO SELECT CITY FROM USER LOCATION============================================================== */
//   useEffect(() => {
//     if (locationAutoSelectedRef.current) return;
...
//       (option) =>
//         normalizeCity(option) === detectedCity
//     );
...
//           <img src="/main.jpg" alt="Luxury Commercial Architecture" className="absolute inset-0 h-full w-full object-cover object-center lg:object-top" />
...
//               <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#A054A0] shadow-[0_0_24px_rgba(160,84,160,.45)]"><Sparkles className="h-3.5 w-3.5 text-white" /></span>
//               <span className="text-[9px] font-bold uppercase tracking-[0.16em] text-white/90 sm:text-[10px] sm:tracking-[0.22em]">Describe · Discover · Decide</span>
//             </div>
...
//                 <div className="grid w-full max-w-[430px] grid-cols-2 rounded-2xl border border-[#ECE7EC] bg-[#F7F5F7] p-1 shadow-[inset_0_1px_2px_rgba(30,15,30,.04)] sm:inline-flex sm:w-auto sm:max-w-none sm:grid-cols-none sm:rounded-full">                  {/* FILTER TAB */}
//                   <button type="button" onClick={() => setTab("filters")} className={`inline-flex h-11 items-center justify-center gap-2 rounded-xl px-4 text-[11px] font-bold transition-all duration-200 sm:h-12 sm:min-w-[160px] sm:rounded-full sm:px-6 sm:text-xs ${tab === "filters"
...
//                   <h2 className="text-lg font-bold tracking-[-0.025em] text-[#191519] sm:text-xl lg:text-2xl">Find your perfect place</h2>
//                   <p className="text-xs leading-5 text-[#8B838B] sm:text-sm">Refine the essentials or describe your workspace in natural language.</p>
//                 </div>
...
//                           <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]">City</label>
//                           <button type="button" onClick={() => setCityOpen(!cityOpen)}
//                             className={`${inputBase} flex items-center justify-between px-3.5 text-left ${cityOpen ? "border-[#A054A0] bg-white ring-[3px] ring-[#A054A0]/10" : ""}`}>
//                             <span className="flex min-w-0 items-center gap-2.5">
//                               <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] border border-[#E9E1E9] bg-white shadow-sm"><MapPin className="h-4 w-4 text-[#A054A0]" /></span>
//                               <span className={`truncate ${city ? "font-semibold text-[#211A21]" : "text-black/35"}`}>
//                                 {loadingCities ? "Loading cities..." : city || "Select City"}</span>
//                             </span>
//                             <ChevronDown className={`h-4 w-4 shrink-0 text-black/35 transition-transform ${cityOpen ? "rotate-180 text-[#A054A0]" : ""}`} />
//                           </button>
//                           {cityOpen && (
...
//                                   <input value={citySearch} onChange={(e) => setCitySearch(e.target.value)} placeholder="Search city..." autoFocus className="h-10 w-full rounded-xl border border-[#E7DFE7] bg-white pl-9 pr-3 text-sm text-[#211A21] outline-none transition focus:border-[#A054A0] focus:ring-2 focus:ring-[#A054A0]/10" />
//                                 </div>
//                               </div>
...
//                                   <div className="px-4 py-8 text-center text-xs text-black/35">No cities found</div>
//                                 )}
//                               </div>
//                             </div>
//                           )}
//                         </div>
//                         <div>
//                           <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]">Micromarket</label>
//                           <div className="relative">
//                             <select value={selectedMicromarket} disabled={!city || loadingMicromarkets}
...
//                               <option value="">{loadingMicromarkets ? "Loading..." : city ? "All Micromarkets" : "Select City First"}</option>
//                               {micromarkets.map((m) => (
...
//                       <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
//                         <div>
//                           <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]">Property Type</label>
//                           <div className="relative">
...
//                           {officeType.toLowerCase() === "managed office/co-working" ? (
//                             <>
//                               <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]">Required Seats</label>
//                               <div className="relative">
...
//                                 </span>
//                                 <input type="text" inputMode="numeric" value={seats} onChange={handleSeatsChange} placeholder="e.g. 50" className={`${inputBase} pl-14 pr-4 placeholder:text-black/25`} />
//                               </div>
//                             </>
//                           ) : (
//                             <>
//                               <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]"> Min Area ({areaUnit}) </label>
//                               <div className="relative">
//                                 <span className="absolute left-3.5 top-1/2 z-10 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg border border-[#E9E1E9] bg-white">
//                                   <Maximize2 className="h-4 w-4 text-[#A054A0]" />
//                                 </span>
//                                 <input type="text" inputMode="numeric" value={area} onChange={handleAreaChange} placeholder="e.g. 2,500" className={`${inputBase} pl-14 pr-4 placeholder:text-black/25`} />
//                               </div>
//                             </>
//                           )}
//                         </div>
//                       </div>
//                       <div>
//                         <label className="mb-2 ml-1 block text-[9px] font-bold uppercase tracking-[0.15em] text-black/50 sm:text-[10px]">Budget ({currency})</label>
//                         <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
//                           <div className="relative">
//                             <span className="absolute left-3.5 top-1/2 z-10 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg border border-[#E9E1E9] bg-white">
//                               {renderCurrencyIcon()}
//                             </span>
//                             <input type="text" inputMode="numeric" value={minBudget} onChange={handleMinBudgetChange} placeholder="Min Budget" className={`${inputBase} pl-14 pr-4 placeholder:text-black/25`} />
//                           </div>
//                           <div className="relative">
//                             <span className="absolute left-3.5 top-1/2 z-10 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg border border-[#E9E1E9] bg-white">                             {renderCurrencyIcon()}</span>
//                             <input type="text" inputMode="numeric" value={maxBudget} onChange={handleMaxBudgetChange} placeholder="Max Budget" className={`${inputBase} pl-14 pr-4 placeholder:text-black/25`} />
//                           </div>
//                         </div>
//                       </div>
//                     </div>
...
//                     <div className="space-y-4">
//                       <div className="relative">
//                         <div className="pointer-events-none absolute left-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-xl border border-[#A054A0]/10 bg-[#A054A0]/10"><Sparkles className="h-4 w-4 text-[#A054A0]" /></div>
//                         <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Tell us what kind of workspace you're looking for..." rows={4} className="min-h-[150px] w-full resize-none rounded-2xl border border-[#E8E0E8] bg-[#FAF8FA] p-4 pl-16 text-sm font-medium leading-6 text-[#211A21] outline-none transition-all duration-200 placeholder:text-black/30 hover:border-[#D4C4D5] hover:bg-white focus:border-[#A054A0] focus:bg-white focus:ring-[3px] focus:ring-[#A054A0]/10 sm:min-h-[160px]" />
...
//                         {SAMPLE_PROMPTS.map(
//                           (samplePrompt, idx) => (
//                             <button type="button" key={idx} onClick={() => setPrompt(samplePrompt)} className="max-w-[300px] shrink-0 truncate rounded-full border border-[#E8E0E8] bg-[#FAF8FA] px-3.5 py-2 text-[11px] text-[#514751] transition-all hover:border-[#A054A0]/30 hover:bg-[#A054A0]/[.06] hover:text-[#A054A0] sm:text-xs">
//                               {samplePrompt}
//                             </button>
//                           )
//                         )}
...
export default function HeroSection({ consentGranted, locationData }) {
   ...
   *
   * These are used for:
   * - Rent/month for non-coworking
   * - Seat Price/month for coworking
   */
  const [minBudget, setMinBudget] = useState("");
  const [maxBudget, setMaxBudget] = useState("");
  ...
  /*
   * Every type except Managed Office/Co-working
   * uses Rent/month.
   *
   * Therefore:
   *
   * All Office Types      -> Rent
   * Conventional          -> Rent
   * Consulting            -> Rent
   * Others                -> Rent
   * Managed/Co-working    -> Seat Price
   */
  ...
  const handleOfficeTypeChange = (e) => {
    ...
    /*
     * Reset dynamic fields whenever property type changes
     * so values from one property type do not carry over
     * into another.
     */
    setMinBudget("");
    setMaxBudget("");
  ...
  //     if (isRentBased && maxBudget) {
  //       params.set(
  //         "maxBudget",
  //         maxBudget.replace(/,/g, "")
  //       );
  //     }
  //     if (isCoworking && seats) {
  //       params.set(
  //         "seats",
  //         seats.replace(/,/g, "")
  //       );
  //     } else if (!isCoworking && area) {
  //       params.set(
  //         "area",
  //         area.replace(/,/g, "")
  //       );
  ...
  const handleSearch = (e) => {
    ...
    if (tab === "filters") {
      ...
      /* =====================================================
         ...
         IMPORTANT:
         These must ALWAYS be captured regardless of
         office type.
  
         Conventional / Consulting / Others:
           minBudget = Min Rent/month
           maxBudget = Max Rent/month
  
         Managed Office/Co-working:
           minBudget = Min Seat Price/month
           maxBudget = Max Seat Price/month
      ===================================================== */
      ...
      if (isCoworking) {
        ...
        // Never carry area into coworking searches
        params.delete("area");
      } else {
        ...
        // Never carry seats into non-coworking searches
        params.delete("seats");
      }
      ...
      /* =====================================================
         ...
         Keep the actual currency code in the URL.
         Example:
           currency=INR
           currency=USD
           currency=AED
